...
{
	public class ManagerEmployeesViewModel : ViewModelBase
	{
		private readonly UnitOfWork _unitOfWork = UnitOfWork.GetInstance();
		private readonly IUIVisualizerService _visualizer;
		public ManagerEmployeesViewModel()
		{
			_visualizer = DependencyResolver.Resolve<IUIVisualizerService>();
		...
		{
			_visualizer.ShowDialog(new AddEmployeeViewModel());
			RefreshEmployeesCollection();
		}
		...
		{
			_visualizer.ShowDialog(new AddEmployeeViewModel(SelectedEmployee));
			RefreshEmployeesCollection();
		}
